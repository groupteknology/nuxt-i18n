...
export function generateTypes(ctx: NuxtI18nContext) {
    ...
    lines.push('type InterpolationValue = boolean | null | number | string | undefined')
    lines.push('type Primitive = boolean | null | number | string | undefined')
    lines.push('type DeepValue<T, Path extends string> = Path extends `${infer Key}.${infer Rest}` ? Key extends keyof T ? DeepValue<T[Key], Rest> : never : Path extends keyof T ? T[Path] : never')
    lines.push('type DeepPartial<T> = T extends Primitive ? T : { [K in keyof T]?: DeepPartial<T[K]> }')
    ...
    lines.push('type ExtractInterpolationKeys<T> = T extends string ? T extends `${string}{{${infer Key}}}${infer Rest}` ? Key | ExtractInterpolationKeys<Rest> : T extends `${string}{${infer Key}}${infer Rest}` ? Key | ExtractInterpolationKeys<Rest> : never : never')
    lines.push('type LocaleShape<T> = T extends string ? string : T extends Primitive ? T : { [K in keyof T]: LocaleShape<T[K]> }')
    lines.push('type ParamsShape<T> = [ExtractInterpolationKeys<T>] extends [never] ? never : { [K in ExtractInterpolationKeys<T>]: InterpolationValue }')
    ...
    lines.push(`${TAB}export type LocaleInput = DeepPartial<LocaleShape<LocaleInstance>>`)
    lines.push(`${TAB}export type LocaleMessage<Path extends LocalePath> = DeepValue<LocaleInstance, Path>`)
    lines.push(`${TAB}export type LocaleParams<Path extends LocalePath> = ParamsShape<LocaleMessage<Path>> extends never ? never : Prettify<ParamsShape<LocaleMessage<Path>>>`)
    ...
    lines.push(`${TAB}${TAB}$t: <Path extends import('@groupteknology/nuxt-i18n').LocalePath>(path: Path, ...args: import('@groupteknology/nuxt-i18n').LocaleParams<Path> extends never ? [] : [params: import('@groupteknology/nuxt-i18n').LocaleParams<Path>]) => string`)
    lines.push(`${TAB}}`)
    lines.push('}')
    lines.push('')
    lines.push(`declare module 'nuxt/app' {`)
    lines.push(`${TAB}interface NuxtApp {`)
    lines.push(`${TAB}${TAB}$locale: import('vue').WritableComputedRef<import('@groupteknology/nuxt-i18n').LocaleCode, import('@groupteknology/nuxt-i18n').LocaleCode>`)
    lines.push(`${TAB}${TAB}$locales: import('@groupteknology/nuxt-i18n').Locales`)
    lines.push(`${TAB}${TAB}$setLocale: (code: import('@groupteknology/nuxt-i18n').LocaleCode) => void`)
